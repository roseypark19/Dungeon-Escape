...
class RotationAnimator {
    ...
    constructor(spriteArray, width, height, frameCount, frameDuration, reverse, loop) {
        Object.assign(this, { spriteArray, width, height, frameCount, frameDuration, reverse, loop });
        this.elapsedTime = 0;
        this.totalTime = this.frameCount * this.frameDuration;
    };
    ...
    drawFrame(tick, ctx, x, y, scale) {
        ...
        let frame = this.currentFrame();
        if (this.reverse) frame = this.frameCount - frame - 1;
        ctx.drawImage(this.spriteArray[frame], 0, 0, this.width, this.height, x, y, this.width * scale, this.height * scale);
    };
